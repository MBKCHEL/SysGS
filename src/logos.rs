...
pub fn get_logo(lang: &str) -> (Vec<ColoredString>, usize, Box<dyn Fn(&str) -> ColoredString>) {
    let lang_lower = lang.to_lowercase();

    let (raw_logo, color_func): (&str, Box<dyn Fn(&str) -> ColoredString>) = match lang_lower.as_str() {
        ...
        "asm" | "assembly" | "assemblygas" => (include_str!("../assets/asm.txt"), Box::new(|s| s.magenta().bold())),
        "c#" | "csharp" => (include_str!("../assets/csharp.txt"), Box::new(|s| s.purple().bold())),
        "php" => (include_str!("../assets/php.txt"), Box::new(|s| s.purple().bold())),
        "java"  => (include_str!("../assets/java.txt"), Box::new(|s| s.red().bold())),
        "ruby"  => (include_str!("../assets/ruby.txt"), Box::new(|s| s.red().bold())),
        "typescript" | "ts" => (include_str!("../assets/typescript.txt"), Box::new(|s| s.blue().bold())),
        "python" | "py" => (include_str!("../assets/python.txt"), Box::new(|s| s.blue().bold())),
        "javascript" | "js" => (include_str!("../assets/javascript.txt"), Box::new(|s| s.yellow().bold())),
        "kotlin" => (
        ...
        _ => (include_str!("../assets/default.txt"), Box::new(|s| s.white().bold())),
    };
